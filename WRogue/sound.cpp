...
void Sound::initialise (void) {
    ...
    if (possible) result = FMOD_System_Init(fmodsystem,10, FMOD_INIT_NORMAL, 0);
    if (result != FMOD_OK) possible = false;
...
void Sound::createMenuSound(const char * filename){
     currentSound = (char *)filename;
        result = FMOD_Sound_Release(sound);
        result = FMOD_System_CreateStream(fmodsystem,currentSound, FMOD_SOFTWARE, 0, &sound1);
        if (result != FMOD_OK) possible = false;
}
...
void Sound::playOnce () {
    if (possible && on) {
        result = FMOD_System_PlaySound(fmodsystem,sound1, NULL, false, &channel);
